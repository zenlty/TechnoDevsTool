...
chcp 857 
cls
title TechnoDevs / GsmTurkey TOOL V1.8
cd "%~dp0"
...
echo( 
echo ******************************************************************
echo *                %[LCYAN]%TechnoDevs / GsmTurkey Tool V1.8%[WHITE]%                * | cmdcolor
...
echo(	
echo  %[LMAGENTA]%1. RECOVERY ISLEMLERI%[LRED]%    %[WHITE]%     %[LYELLOW]%       9.  SIFIRLAMA MENUSU  %[WHITE]%         %[LMAGENTA]%17. IMAJ OLUSTUR      %[WHITE]%   | cmdcolor 
...
echo  %[LMAGENTA]%5. TOOL'UN EKRANINI YENILE%[LRED]%      %[WHITE]%     13. %[LYELLOW]%CIHAZ BILGILERINI OKU %[WHITE]%     %[LCYAN]%21. DIAG ACMA      %[WHITE]%      | cmdcolor
...
echo  %[LCYAN]%7. FASTBOOT DRIVER KUR %[WHITE]%             %[LRED]% 15. TR DIL EKLE%[LRED]%       %[WHITE]%         %[LGREEN]%23. DEGISIKLIK LISTESI      %[WHITE]%               | cmdcolor

echo  %[LRED]%8. FRP GOOGLE KILIDINI SIL %[WHITE]%       %[LCYAN]%   16. ARAMA YAPTIRMA %[WHITE]%            %[LRED]%24. PREMIUM OZELLIKLER      %[WHITE]%        | cmdcolor
echo.
echo              %[LYELLOW]%G. TOOL HAKKINDA / MENULER / GENEL BILGILER               %[WHITE]%         | cmdcolor
echo.
...
if /I %option%==0044555555 goto twrpportlayici
if /I %option%==16 goto aramayaptirma
if /I %option%==G goto arachakkinda
if /I %option%==Kilavya590044555555 goto carliv
...
if /I %option%==23 goto changelog
if /I %option%==24 goto premiumozellikler
...
echo  %[LCYAN]%PREMIUM VERSION NE BARINDIRIR ?%[WHITE]%         | cmdcolor
...
echo %[LGREEN]%UCRETI NE KADAR NASIL KULLANIRIM %[WHITE]%         | cmdcolor
...
echo  %[LMAGENTA]% PREMIUM MENUSUNE GOZ ATMAYI UNUTMAYIN%[WHITE]%         | cmdcolor
...
echo    %[LYELLOW]% 3.  UCUNCU ADIM ( PORT ) %[WHITE]%         | cmdcolor
...
:paketletwrp
echo %[LRED]%TWRP RECOVERY PORTLAMAK ICIN GEREKLI SARTLAR ICIN 1'E BASINIZ%[WHITE]% | cmdcolor
echo %[LRED]%ACILACAK MENUDEN PORT RECOVERY SECINIZ.%[WHITE]% | cmdcolor
echo %[LRED]%ARDINDAN PAKETLE BUTONUNU SECINIZ.%[WHITE]% | cmdcolor
echo %[LRED]%TWRP RECOVERY HAZIR OLARAK KLASOR ICINDE ACILACAKTIR%[WHITE]% | cmdcolor
pause
goto recovery
:ucuncuadim
...
@echo Yedeklencek
pause
xcopy "C:\Tool\portlayici\recovery-stock\recovery.img-kernel" "C:\Tool\portlayici\recovery-port" /s
xcopy "C:\Tool\portlayici\recovery-stock\ramdisk\fstab.qcom" "C:\Tool\portlayici\recovery-port\ramdisk" /s
pause
goto twrpportlayici
:ikinciadimtwrpport
cls
...
start explorer.exe "C:\tool\portlayici\recovery-resources"
...
echo %[LRED]%PORTLAYICIYA GERI DONUN VE IKINCI ADIMA GECIN%[WHITE]% | cmdcolor
...
echo(
echo ][*************************][*************************][
cecho ][  {0B}1. AYIKLA (ILK OLARAK BUNU SEC)
echo ][*************************][*************************][
cecho ][  {0B}2. PAKETLE / TOPLA{#}    T: TWRP PORTLAYICIYA DON    
echo ][*************************][*************************][
cecho ][ AYIKLADIKTAN SONRA {0D} R. BASKA RECOVERY SEC{#}                  ][{\n}
echo ][*************************][*************************][
echo ][                3 .UCUNCU ADIMA GEC                ][
echo ][*************************][*************************][
echo(
...
set /p noboot=There is no image in your [boot-resources] folder. Place some in there and then press [B] to start again or [Q] to go to start menu, then press ENTER: || set noboot="0"
...
echo  %[LCYAN]% IKINCI OLARAK PORT RECOVERY'I AYIKLAYIN0               %[WHITE]%         | cmdcolor
...
set /p norec=There is no image in your [recovery-resources] folder. Place some in there and then press [R] to start again or [Q] to go to start menu, then press ENTER: || set norec="0"
...
pause
start explorer.exe "C:\tool\portlayici\output"
goto imgmenu
:img_info
cls
